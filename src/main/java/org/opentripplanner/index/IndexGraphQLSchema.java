    ...
    public IndexGraphQLSchema(GraphIndex index) {
        ...
        carParkType = GraphQLObjectType.newObject()
                ...
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        ...
                        .type(Scalars.GraphQLInt)
                        .dataFetcher(environment -> ((CarPark) environment.getSource()).maxCapacity == Integer.MAX_VALUE ? -1 : ((CarPark) environment.getSource()).maxCapacity)
                        .build())
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        .name("spacesAvailable")
                        .description("Number of currently available parking spaces at the car park")
                        .type(Scalars.GraphQLInt)
                        .dataFetcher(environment -> ((CarPark) environment.getSource()).spacesAvailable == Integer.MAX_VALUE ? -1 : ((CarPark) environment.getSource()).spacesAvailable)
                        .build())
                .field(GraphQLFieldDefinition.newFieldDefinition()
                        .name("realtime")
                        .description("If true, value of `spacesAvailable` is updated from a real-time source.")
		            ...
		            .field(GraphQLFieldDefinition.newFieldDefinition()
		                    ...
		                    .dataFetcher(environment -> {
		                        ...
		                        return index.findClosestStopsFlyCrow(lat, lon, range)
		                        		...
		                        		.sorted(Comparator.comparing(zone -> (int) zone.getIsContainsZone()))
		                                .flatMap(zone -> {
		                                	return index.fareRulesById.values()
                                        	.stream()
                                        	.flatMap(Collection::stream)
                                        	.map(fareRule -> {
